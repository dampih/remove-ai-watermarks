"""Tests for the Dola AI visible-watermark engine (localize -> fill)."""
...
from remove_ai_watermarks.dola_engine import (
    _ALPHA_HEIGHT_FRAC,
    _ALPHA_WIDTH_FRAC,
...
# The engine sizes the glyph from the SHORT side (scale_basis="short").
_BASE = 1024

SAMPLE = Path(__file__).resolve().parents[1] / "data" / "fixtures" / "visible" / "dola" / "example.png"


def _compose(w: int, h: int, bg: float = 100.0):
    """Composite the real alpha (scaled to short side ``s``) onto a flat bg.
    Returns ``(watermarked_uint8, mark_bool_mask)``."""
    img = np.full((h, w, 3), bg, np.float32)
    at = _alpha_template()
    short = min(w, h)
    gw, gh = int(_ALPHA_WIDTH_FRAC * short), int(_ALPHA_HEIGHT_FRAC * short)
    margin = int(0.02 * short)
    ax = w - margin - gw
    ay = h - margin - gh
...
class TestLocate:
    ...
    def test_box_scales_with_short_side(self):
        eng = DolaEngine()
        small = eng.locate(np.zeros((1024, 2048, 3), np.uint8))
        large = eng.locate(np.zeros((2048, 4096, 3), np.uint8))
        # short side doubles: the box must double too (short-side basis).
...
class TestDetect:
    ...
    def test_match_score_shape_sensitive(self):
        ...
        sil = _glyph_silhouette()
        gw = int(_ALPHA_WIDTH_FRAC * _BASE)
        gh = int(_ALPHA_HEIGHT_FRAC * _BASE)
        # box that contains the silhouette AT TEMPLATE SCALE -> high score
        box = np.zeros((gh + 8, gw + 8), np.uint8)
        box[4 : 4 + gh, 4 : 4 + gw] = cv2.resize(sil, (gw, gh))
        assert _template_match_score(box, _BASE) >= DETECT_NCC_THRESHOLD
        # a uniformly filled box has no glyph structure -> low score
        solid = np.full_like(box, 255)
        assert _template_match_score(solid, _BASE) < DETECT_NCC_THRESHOLD
    ...
    def test_small_image_guarded_from_false_positive(self):
        ...
        Bracket it: a real mark is detected at native size, but the same content
        downscaled below the guard is not."""
        wm, _mark = _compose(_BASE, _BASE)
        eng = DolaEngine()
...
@pytest.mark.skipif(not SAMPLE.exists(), reason="generated example image not present")
class TestGeneratedExample:
    def test_detects_watermark(self):
        det = DolaEngine().detect(load_image_bgr(SAMPLE))
        assert det.detected
        assert det.confidence >= DETECT_NCC_THRESHOLD

    def test_fill_lowers_confidence(self):
        img = load_image_bgr(SAMPLE)
        before = DolaEngine().detect(img)
        assert before.detected
        out, region = registry.get_mark("dola").remove(img, backend="cv2")
        assert region is not None
        assert DolaEngine().detect(out).confidence < before.confidence

    def test_far_region_untouched(self):
        img = load_image_bgr(SAMPLE)
        out, _ = registry.get_mark("dola").remove(img, backend="cv2")
        h, w = img.shape[:2]
...
class TestFootprintMaskAndRemoval:
    def test_footprint_mask_in_bottom_right(self):
        ...
        wm, _mark = _compose(_BASE, _BASE)
        mask = DolaEngine().footprint_mask(wm)
    ...
    def test_removes_synthetic_mark(self):
        ...
        wm, _mark = _compose(_BASE, _BASE)
        assert DolaEngine().detect(wm).detected
        out, region = registry.get_mark("dola").remove(wm, backend="cv2")
        assert region is not None
        assert not DolaEngine().detect(out).detected
    ...
    def test_fill_removes_and_leaves_far_region(self, w, h):
        ...
        before = DolaEngine().detect(wm)
        out, _ = registry.get_mark("dola").remove(wm, backend="cv2")
        assert DolaEngine().detect(out).confidence < before.confidence
...
class TestDegenerateAndChannelInputs:
    ...
    def test_locate_box_stays_in_bounds(self, shape):
        ...
        from remove_ai_watermarks.dola_engine import _CONFIG

        h, w = shape[:2]
        img = np.zeros(shape, np.uint8)
        loc = TextMarkEngine(_CONFIG).locate(img)
        assert loc.x >= 0
        assert loc.y >= 0
